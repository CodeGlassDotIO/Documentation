...
Before the `find_items` call, we add the `cgprofile` macro defined in the [CodeGlass.jl](../../languages/Julia/codeglass-jl) package. This will automatically create a [recording](../../concepts-and-features/datasources#recordings) of the call, allowing us to focus on just the code that we are interested in.
